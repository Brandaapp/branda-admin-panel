...
export default function ShuttleForm(props) {
    ...
    function submit() {
        axios.patch(`/api/shuttles/${props.date.toISOString()}`, {
            start: state.start,
            end: state.end,
            busID: state.busID,
                ...
                <div className="popup-form">
                    ...
                    <div className="shuttle-form-input">
                        <Input
                            ...
                            type="text"
                            value={state.busName}
                            onChange={(e) => setState(prev => ({ ...prev, busName: e.target.value }))}
                        />
                    </div>
                    ...
                    <div className="shuttle-form-input">
                        <Input
                            ...
                            type="text"
                            value={state.busID}
                            onChange={(e) => setState(prev => ({ ...prev, busID: e.target.value }))}
                        />
                    </div>
                    <div className="row">
                        <div className="col s6">
                            ...
                            <KeyboardTimePicker
                                value={state.start}
                                onChange={date => setState(prev => ({ ...prev, start: date.toJSDate() }))}
                            />
                        </div>
                        <div className="col s6">
                            ...
                            <KeyboardTimePicker
                                value={state.end}
                                onChange={date => setState(prev => ({ ...prev, end: date.toJSDate() }))}
                            />
                        </div>
                    </div>
                    <div className="btn-row">
                        <button className="btn waves-effect waves-light shuttle-form-cancel-btn" onClick={cancel}>
                            Cancel
                        </button>
                        <button className="btn waves-effect waves-light shuttle-form-btn" type="submit">
                            Submit
                        </button>
                    </div>
                </div>
